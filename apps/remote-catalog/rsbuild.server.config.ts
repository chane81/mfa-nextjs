 ...
 * - CommonJS(`commonjs2`) 라이브러리로 출력: host 서버가
 *   `new Function("module","exports","require", code)` 로 평가한다.
 * - react 계열은 external → host 의 React 인스턴스를 주입받는다. 여기서 React 를
 *   번들에 넣으면 서버에서도 React 가 2벌이 된다.
 *
