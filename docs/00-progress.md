...
## 2026-09-18 (42차) — 호출자에게 시키던 판단을 소유자에게 돌려준다

아키텍처 · SSOT 관점으로 저장소를 훑었다. 나온 것 중 **같은 모양이 넷**이었다 —
**어떤 값을 "누가 고르는가" 가 잘못 놓여 있고, 그래서 고르는 쪽이 틀릴 수 있었다.**

| 값                        | 고르던 쪽            | 옮긴 곳                         |
| ------------------------- | -------------------- | ------------------------------- |
| remote 의 현재 엔트리 URL | 경계의 **호출부 둘** | `RemoteBoundary` 자신           |
| 심어준 버전 객체의 모양   | `RemoteVersionSync`  | `versions/browser` (읽는 쪽 옆) |
| 컨테이너 포트             | 공유 엔트리포인트    | 각 Dockerfile (+ 대조 테스트)   |
| lab 실험의 대상 remote    | 페이지 **두 곳**     | `lab/modes.ts` 상수 하나        |

첫 번째는 리팩터링이 아니라 **실재하는 버그**였다(J-1). 나머지 셋은 "지금은 안 터진다" 인데,
셋 다 **터질 때 실패가 아니라 오작동으로 나타난다.**

### 진단이 거짓말하던 자리 — `RemoteBoundary` 의 `entry`

remote 가 죽으면 에러 상자에 `entry:` 한 줄이 찍힌다. 그게 `WEB_ENTRIES[remote]` 였다 —
**dev 에만 실재하는 폴백 주소**다. 런타임이 실제로 부르는 건 `pinnedEntry(remote)` 이고,
배포에서는 `/v<version>/mf-manifest.json` 이다. 즉 장애가 났을 때 화면에 나오는 주소와
실제로 실패한 주소가 달랐다.

같은 함정을 `MfDiagnostics` 가 먼저 밟고 고쳤는데(주석에 기록까지 남아 있다) 경계는 남아
있었다. 호출부가 둘뿐이었는데 **둘 다 같은 값을 틀리게 넘기고 있었다** — 그게 이 prop 을
없앨 이유다. 이제 경계는 `remoteName` 만 받고 주소는 자기가 고른다.
...
테스트가 그 틀린 값을 **고정하고 있었다는 점**이 더 나쁘다. `RemoteComponent.test.tsx`
안에서 경계 테스트는 버전 없는 주소를, 바로 아래 lazy 캐시 테스트는 `/vt9zzz/…` 를 각각
옳다고 단언했다. 경계 쪽만 주입 스텁을 안 불러서 그 어긋남이 한 파일 안에서도 안 보였다.

### 심는 쪽과 읽는 쪽이 이름만 공유하고 모양은 아니었다

`REMOTE_VERSIONS_GLOBAL`(전역 **이름**)은 원래부터 한 곳이었다. 그런데 그 전역에 들어가는
`{ version, entry }` **모양**은 `RemoteVersionSync` 안의 객체 리터럴이었고 `InjectedEntry` 와
타입으로 묶여 있지 않았다. 필드 이름이 하나 어긋나면 컴파일은 통과하고 `?.entry` 만
`undefined` 가 된다 — 증상은 에러가 아니라 **버전 없는 폴백으로 조용히 붙는 것**(G-1 그 모양).

직렬화를 `versions/browser.ts` 로 옮겨 읽는 쪽 옆에 뒀다. 그러면 테스트가 붙는다 —
`injectionScript` 가 뱉은 문자열을 실행해 `injectedEntry` 로 되읽는 **왕복 한 개**가 이
seam 전체를 덮는다. 한쪽만 시험하면 어긋나도 양쪽이 다 초록이다.

### MF 초기화에 테스트가 하나도 없었다

`loader/index.ts` 는 `mf/**` 에서 테스트가 없는 유일한 소스였다. seam 이 한 칸 위에 있었다 —
정규화 프로브(`react-modules.ts`)에는 89줄짜리 테스트가 있는데, 이 저장소가 실제로 기록한
사고는 그 규칙 안이 아니라 **"규칙이 다섯 개 전부에 똑같이 입혀졌는가"** 쪽이다. 그 파일
주석이 직접 그렇게 적어 뒀다("하나만 다르게 적혀도 그 모듈만 조용히 싱글턴에서 빠진다").

그래서 순수 함수가 아니라 **`init()` 이 실제로 받은 인자**를 본다 — shared 5키와 공통 설정,
`remotes` 가 `pinnedEntry` 를 쓰는지, 초기화가 한 번인지, `typeof window` 두 갈래.

### Docker · 셸 계층을 참조가 아니라 **대조**로 묶는다

SSOT 점검 결과가 한 줄로 요약됐다 — **TS 세계는 `REMOTES` 한 곳만 고치면 끝나고,
Docker · compose · 셸은 여전히 손으로 따라 고쳐야 한다.** 그건 어쩔 수 없다(YAML 은 import
를 못 한다). 고칠 수 있는 건 **어긋난 걸 아무도 안 본다는 쪽**이다.

`scripts/docker-ssot.test.ts` 가 `REMOTE_LIST` 를 순회하며 compose 의 포트 · 헬스체크가 찌르는
파일명 · 오리진 env 이름, 각 Dockerfile 의 `ENV PORT`/`EXPOSE` 를 SSOT 와 맞춘다. 오프라인이고
**어느 파일의 어느 값을 무엇으로 바꿔야 하는지 말해준다.** 검사가 진짜로 잡는지 확인하려고
포트 · 매니페스트 이름 · 폴백을 일부러 어긋내 봤다(검증일: 2026-09-18).

```
compose 포트 3002→3003 · 헬스체크 mf-version.json→version.json · PORT 폴백 복구
  → 3 failed | 9 passed     복원 후 12 passed
```

같이 엔트리포인트의 `${PORT:-3001}` 도 없앴다. 두 remote 가 같이 쓰는 스크립트에서 기본값은
곧 어느 한쪽의 포트다(J-2).
...
| 무엇                             | 판단                                                                     |
| -------------------------------- | ------------------------------------------------------------------------ |
| `ProductDetailSection` (13줄)    | 삭제. 콜백이 없어 `RemoteComponent` 위임 한 줄이 전부였다                |
| `hueVar` (`@mfa/ui` 비공개)      | `@mfa/utils/style/hue` 로. `--hue` 라는 **이름이 계약**인데 3곳이 베꼈다 |
| `clearCookies` (테스트 7벌 복붙) | `tests/helpers/cookies.ts` 로. 어긋나면 순서 의존 실패로만 드러난다      |
| `@mf-types` 경로 (쓰는·읽는 쪽)  | `CONTRACTS_TYPES_DIR` 하나로. 어긋나면 안내 메시지가 엉뚱해진다          |
| lab 의 `'catalog'` 리터럴 2곳    | `LAB_REMOTE` 하나로. 갈리면 비교 실험이 조용히 무의미해진다              |

### `hueVar` 를 어디에 둘 것인가 — 한 번 틀렸다

처음에는 `@mfa/ui` 에서 export 했다. 리뷰에서 "`packages/utils` 가 맞지 않나" 가 나왔고,
**그쪽이 맞았다.** 내 판단이 ADR-025 를 좁게 읽은 것이다 — 그 기록이 패키지를 테스트 헬퍼
패키지로 기술하고 있어서 "테스트 전용" 으로 읽었는데, 정작 그 ADR 의 마지막 줄은
"프로덕션 유틸이 들어와도 배포 판별이 이미 맞다" 였다.

기록이 범위를 좁게 말하면 다음 사람이 그 범위로 판단한다. ADR-026 으로 선을 다시 그었다 —
`@mfa/utils` 는 **소유자가 없는 범용 유틸**의 자리고 `src/test/` 는 그중 한 갈래다.

`@mfa/ui` 가 틀린 이유는 따로 있다. 그 패키지는 **컴포넌트를 내보내는 자리**인데,
소비처 셋 중 둘은 프리미티브를 안 쓰면서 이 값만 필요하다.

`ProductDetailSection` 이 남긴 규칙: **콜백이 필요할 때만 Section 을 만든다.** 그래야 새
remote 모듈 하나에 세는 파일 수가 정직해진다(ADR-020 의 연장).

`exposedNames` 도 삭제 후보로 올라왔지만 남겼다 — "모든 remote 가 최소 한 개를 노출한다" 는
진짜 성질 검사가 그걸 쓴다. 지워도 복잡도가 모이지 않고 필터가 테스트로 옮겨갈 뿐이다.

### 남은 것

| 후보                                               | 왜 안 했나                                      |
| -------------------------------------------------- | ----------------------------------------------- |
| MF 플러그인 옵션 7종이 번들러별로 두 벌            | 플러그인 계열·메이저가 달라 타입 공유가 안 된다 |
| `server-entry.test.tsx` 두 벌                      | 헬퍼로 접을 수 있다. remote 3개째가 생기면 한다 |
| `state/` 의 동사 규약 4종 · `'remote-versions'` 셀 | 배치 변경이라 ADR 이 먼저다                     |
| host 포트 3000 에 SSOT 가 없다                     | `REMOTES` 에 host 가 없는 건 설계다 — 결정 필요 |
