...
**무언가를 더 나은 것으로 교체했는데, 옛것이 덤으로 갖고 있던 성질 하나가 같이 안 따라왔다.**
교체 자체는 다 옳았고 테스트도 초록이었다. 빠진 건 아무도 안 적어둔 부수 효과 쪽이다.

| 교체                                    | 같이 빠진 성질                     | 언제 드러나나                       |
| --------------------------------------- | ---------------------------------- | ----------------------------------- |
| bash 판별 → `deploy-targets.ts`         | 러너 Node 버전 고정                | 러너 이미지가 갱신되는 날           |
| `concurrently` → `serve-all-remotes.ts` | `--kill-others` 의 **시그널 전달** | CI 취소 · turbo 가 태스크를 죽일 때 |
| 글롭 집합 완전일치 → 정규식 파서        | 모르는 글롭에서 **크게 실패하기**  | 워크스페이스 글롭이 늘 때           |

셋 다 "지금은 안 터진다" 가 맞다. 그리고 셋 다 **터지는 조건이 우리 커밋 밖에 있다.**

### 배포 전체를 막을 수 있던 자리 — `detect` job 의 Node

가장 나쁜 것 하나. `.github/actions/detect-targets` 가 `node …/deploy-targets.ts` 를 부르는데
그 job 에 런타임을 고정하는 스텝이 없었다. `.ts` 직접 실행은 타입 스트리핑에 기대는 것이라,
러너 이미지에 딸려 오는 Node 가 옛 버전이면 `ERR_UNKNOWN_FILE_EXTENSION` 하나로 끝난다.
`detect` 가 죽으면 `deploy` 의 나머지 job 은 전부 안 뜬다 — remote 하나가 아니라 배포 전체다.

`pnpm install` 을 안 받는 걸 이 액션의 장점으로 삼다가 런타임 고정까지 같이 뺀 것이다.
**"의존성이 필요 없다" 와 "런타임이 아무거나 되어도 된다" 는 다른 말이다.**
`pnpm/setup@v2` 를 `install: false` 로 넣어 고쳤다 — 의존성은 여전히 안 받고 런타임만
ci.yml 세 job 과 같은 범위 표현(`node@^24.19.0`)으로 고정한다. 증상과 근거는 I-12.

### 검사가 스스로 약해지던 자리

`scripts/docker-context.test.ts` 의 파서가 `'<dir>/*'` 모양만 골라 담고 나머지는 **건너뛰고**
있었다. `- 'tools/**'` 같은 항목이 늘면 그 디렉터리만 조용히 빠지는데, 남은 항목이 있으니
개수 검사(`dirs.length > 0`)는 통과한다. 그 밑의 패키지가 Dockerfile 목록과 한 번도
대조되지 않는 상태 — **이 테스트가 막으려던 I-10 드리프트 그 자체가 테스트 안에서 재발한다.**

대체되기 전의 옛 테스트는 글롭 집합을 완전일치로 봐서 그 상황에서 크게 실패했다. 즉 이건
"검사를 SSOT 에서 읽게 바꿨다" 의 대가로 치른 것이고, 치를 이유가 없는 대가였다.
못 읽는 모양을 만나면 던지도록 바꾸고, 스캔 범위도 `packages:` 블록으로 좁혔다 —
파일 전체에 돌리면 `onlyBuiltDependencies` 의 항목까지 워크스페이스 글롭으로 오인한다.

### 나머지 셋

| 무엇                                    | 고침                                                      |
| --------------------------------------- | --------------------------------------------------------- |
| `serve-all-remotes` 가 시그널을 안 넘김 | SIGINT · SIGTERM 을 자식에게 전달 + 5초 뒤 강제 종료      |
| warm 의 `lazy` 캐시가 무한히 자람       | 키를 쌓지 않고 **remote 당 한 칸**만 쓴다                 |
| 문서 · 주석이 `concurrently` 를 가리킴  | 시작 가이드 · `ci.yml` · `turbo.json` 을 지금 코드에 맞춤 |

시그널 쪽은 `next build` 가 고아로 남아 `.next` 에 계속 쓰는 자리다. 다음 빌드가 반쯤
쓰인 산출물을 문다. 양쪽 구현에 실제로 SIGTERM 을 보내 확인했다(검증일: 2026-09-06).

```
$ node scripts/serve-all-remotes.ts -- sleep 60 &   # 감독에게만 SIGTERM
고침 전:  감독 종료됨 / 자식 sleep 고아로 남음
고침 후:  감독 종료됨 / 자식도 같이 종료됨
```

warm 쪽은 `RemoteComponent` 의 `lazyCache` 에 있던 결함을 두 번째
캐시로 복제한 꼴이었다 — nonce 가 요청마다 유일하니 웹훅 한 번에 엔트리가 하나씩 는다.

### 테스트

축출과 파서는 **코드를 읽어서는 회귀를 알아채기 어렵다**(하나는 몇 주 뒤 메모리로만,
하나는 조용한 통과로만 나타난다). 그래서 둘 다 테스트를 붙였다 — 캐시 크기를 직접 보는
것 2개, 파서의 블록 한정·throw·`packages:` 부재 3개. test-plan 41 · 43 번.

38차에 들어온 `deploy-targets.test.ts` 와 PR #10 의 `docker-context.test.ts` 가 진척도 표에
빠져 있어서 같이 채웠다(41 · 42번).

그 캐시 테스트를 넣다가 **로컬만 초록인 상태**를 하나 밟았다. `vitest.config.ts` 의 alias
목록에 `@mfa/contracts/remote` 가 없어서 `exports` 를 타고 `dist` 로 떨어졌는데, 로컬에는
빌드해 둔 `dist` 가 있어 그냥 풀렸다. CI 는 fresh clone 이라 거기서만 죽었다 — 677개 중
그 파일 하나. alias 를 추가하고 모킹을 걷어내 진짜 `MODULE_IDS` 를 쓰게 했다(I-13).
`dist` 를 치우고 돌리면 재현된다.

### 다음에 할 것

- `deploy.yml` 은 `actions/checkout@v5`, `ci.yml` 은 `@v7` 이다. 이번 범위 밖이라 안 건드렸다
- `packages/remote-config/src/index.ts` 에 import 가 없다는 전제(`deploy-targets.ts` 가 상대
  `.ts` 로 직접 읽는다) 위에 서 있는데, 그 불변식을 지키는 검사가 없다
...
200자를 넘고 최대 367자 — 읽는 사람은 그 전에 그림으로 눈이 간다. 수치·근거는 그대로
두고 **배치만** 바꿨다(본문 글자수 10,103 → 9,925).

### 형식

| 자리           | 규칙                                                                             |
| -------------- | -------------------------------------------------------------------------------- |
| h3 바로 아래   | **그 절의 답** 한 줄(`.claim`). 없던 절 다섯(1-1 · 2-1 · 2-3 · 3-2 · 3-3)에 신설 |
| 블록           | 굵은 결론 1줄(`.lead`) + `–` 포인트 2~4개(`.pts`). 인과는 `→`                    |
| 블록 순서      | 답 → 근거 → 대가/예외. 제약·배경은 뒤로 또는 포인트로 흡수                       |
| 함정/오해 블록 | 제목 그대로, 본문을 "무슨 일 → 안 된 답 → 고친 방법 → 교훈"                      |
| figcaption     | 굵은 결론 + 한 문장 부연                                                         |
| FAQ            | 답 첫 문장만 굵게. 순서 재배치 둘("왜 공식 플러그인", "한계")                    |

2-6(상태 공유)이 표본이었다. 처음엔 제약("remote 는 host 하고만")을 첫 블록에 뒀는데,
절 제목이 "어떻게 공유하나"이므로 첫 줄은 **답**(스토어 싱글턴)이어야 했다. 제약은 "왜
공유 패키지냐"의 포인트로 내려갔다. 이 판단이 위 표의 "블록 순서" 규칙이 됐다.

### 검토에서 걸린 것

초안을 `docs/` 밖에 만들어(Pages 가 `docs/` 를 통째로 올리므로 초안이 공개된다) 눈으로
검토한 뒤 반영했다. 반영 전 원문과 초안의 **code 식별자 · 수치 · ADR/이슈 참조 · 강조어**를
집합으로 대조했고, 거기서 하나 잡혔다 — 2-2 에 "같은 `loadRemote()` 호출이 두 갈래로"라고
썼는데 **틀린 말이다**. 서버 경로는 `loadRemote` 를 쓰지 않는다(`new Function` 평가).
도해의 공통 진입은 `loadRemoteModule()` 이다. 원문엔 없던 주장을 요약하면서 보탠 것이라,
문장을 새로 쓸 때는 도해와 다시 맞춰야 한다.

되돌린 것 하나 — 2-5 ③④(해시 · 서명)는 일반 독자용 설명체라 포인트화하지 않고 원문
문단을 유지했다. 1-2 함정 블록 안에 있던 관측 시한 · 폴링 간격 두 덩이는 블록 밖으로
뺐다(함정은 I-8 하나다).

## 2026-09-05 (36차) — 번들러 비교를 해부도에 그림으로 옮긴다

34차에 남긴 [remote 번들러 비교](./01-research/04-bundler-comparison.md)는 마크다운에만
있었고, 해부도에는 FAQ 한 문단이 전부였다. 그런데 이 비교의 핵심은 문장이 아니라
**순서**다 — "shared 배리어가 `import()` 앞이냐 뒤냐". 표로는 그 순서가 안 보인다.
그래서 `PART 3` 을 신설했다.
...
| 절  | 무엇                                                                                                                                  |
| --- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 3-1 | **같은 모듈 하나를 가져오는 순서를 두 줄기로 나란히** — 왼쪽 Vite(배리어가 import 뒤), 오른쪽 Rspack(`ensureChunk` 안의 `f.consumes`) |
| 3-1 | 오해 정정 블록 — "Rsbuild 는 미리 다 컴파일해서"가 아니다(`lazyCompilation` 기본 `imports: true`)                                     |
| 3-2 | 요약 비교표 10행 + **함정 분포표** — Vite 5건(전부 MF 통합 지점) · Rsbuild 1건(설치)                                                  |
| 3-3 | Rsbuild 쪽 대가 4행 + 선택 지침 4행 · catalog 를 옮기지 않는 이유                                                                     |

FAQ 의 같은 항목은 **줄였다**. 두 곳에 같은 분량을 두면 갈린다 — 결론 한 문단만 남기고
PART 3 을 가리킨다. 머리말의 "그 두 축"도 세 축이 됐다.

### 본문 폭 — `.lede` · `.claim` 만 좁았다

`p` 에는 폭 제한이 없어 표·도해와 같은 1160px 인데 `.lede`(62ch)와 `.claim`(66ch)만
좁아서, 그 두 블록에서 줄이 일찍 끊겼다. 35차에 `.note` 의 `max-width` 를 지운 것과
같은 이유로 지웠다. 쓰이지 않던 `.prose`(68ch) 규칙도 같이 지웠다 —
`class="prose"` 사용처가 0건이었다.

### 확인 방법

35차와 같다. SVG 11개를 XML 로 파싱하고, 헤드리스 브라우저로 실제로 열어
**새 도해의 `<text>` 전부를 `getBBox()` 로 레인 경계(20~~470 · 510~~960)와 대조**했다.
넘침 0건. 라이트·다크 양쪽을 눈으로도 봤다 — 색만 다르고 배치는 같다.

좌표 대조를 스크립트로 돌린 것이 35차와 다르다. 35차에는 캡처를 눈으로만 봤는데,
그러면 **아슬아슬하게 걸친 것**을 놓친다 — 실제로 하나 걸렸다. 2-5(신뢰 경계) 도해의
"공개키로는 서명을 만들 수 없다" 줄이 viewBox 폭 940 을 10px 넘겨 오른쪽 끝이 잘려
있었다. 문구를 줄여 고쳤다. **35차의 캡처에도 이미 있던 것이다.**

## 2026-09-04 (35차) — 해부도를 지금 구조로 맞추고, 링크로 열리게 한다

`docs/anatomy.html` 은 **26차에서 멈춰 있었다.** 그 뒤로 계약의 소유권이 뒤집혔고(27차)
배포 순서를 CI 가 쥐게 됐는데(29~32차) 그림은 여전히 "Watch Paths 로 세 앱이 각자 뜬다"
였다. 그림이 코드보다 오래된 것은 **없느니만 못하다** — 읽는 사람이 그걸 근거로 삼는다.

### 고친 것

| 절             | 무엇                                                                          |
| -------------- | ----------------------------------------------------------------------------- |
| 1-1            | `Watch Paths` → `detect` job · `POST /api/application.deploy`, Autodeploy off |
| **1-2 (신설)** | job 넷과 그 조건 · 버전 스큐 표 · 완료 판정을 상태로 바꾼 함정(I-8)           |
| 1-4            | 시퀀스 재배열 — GHA 가 먼저 트리거, 5초 폴링, 버전 확인 30초는 **관측**       |
| 2-1            | 라우트 목록에 `cart/CartBadge` · `catalog/RelatedProducts`                    |
| **2-4 (신설)** | 계약을 remote 가 소유하고 DTS 로 흘러오는 네 단계 · 배럴 순환 함정            |
| 2-6            | 상태 통로가 둘이라는 것(쿠키 · 주소) · `Slot`/`Section` 분리 함정             |
| 1-5            | 저장소 Variables 넷 행 추가 — 폴백 리터럴을 지운 이유(30차)                   |
| 2-5            | 오리진 허용 목록 env 이름 정정 — `ALLOWED_ORIGINS` → `REMOTE_ALLOWED_ORIGINS` |
| FAQ            | 번들러 선택(34차)과 "host 는 remote 의 props 를 어떻게 아나" 둘 추가          |

절이 둘 늘어 **번호가 밀렸다**(1-2~~1-5, 2-4~~2-6). 본문에서 절을 가리키는 자리도 같이
고쳤다 — `grep` 으로 찾지 않으면 조용히 어긋난다(실제로 한 곳 있었다).

### 전수 대조 — 그림에 적힌 값을 코드에서 하나씩 확인했다

"낡았을 것 같은 데"만 고치면 남는다. 그래서 본문 텍스트를 뽑아 **적힌 값 전부**를 저장소와
맞췄다. 대부분 맞았고(`REMOTE_KEEP_VERSIONS` 기본 5, TTL 30초, 재시도 5회, Vite 8 ·
Rsbuild 2, `mfa-cart`, `globalSingleton`, `189,000원`, `주문서`, 세 태그 헬퍼)
**틀린 것은 하나였다** — 오리진 허용 목록의 env 이름. 실제는 `REMOTE_ALLOWED_ORIGINS` 다
(`apps/host/src/mf/trust/index.ts:49`).

같이 넣은 것 둘. 32차의 배포 소요 실측(빌드 p50 catalog 21초 · cart 19초 · host 27초)을
1-2 에, 30차에 필수가 된 저장소 Variables 넷을 1-5 표에 넣었다. **기각·실측이 그림에 없으면
그림만 보는 사람은 그 판단을 다시 하게 된다.**

### 확인 방법 — 눈으로 봤다

SVG 는 손으로 쓴다. 그래서 좌표를 고치면 **글자가 상자 밖으로 나가도 아무도 안 알려준다.**

1. SVG 10개를 XML 로 파싱해 형식이 깨지지 않았는지 봤다.
2. 헤드리스 브라우저로 실제로 열어 새/고친 도해를 캡처했다.

2번에서 세 건 잡혔다. **1번만 했으면 하나도 못 봤다.**

| 무엇                                                  | 고침                              |
| ----------------------------------------------------- | --------------------------------- |
| 2-1 모듈 목록 한 줄이 상자 폭을 넘어 잘렸다           | 두 줄로 갈랐다                    |
| 1-4 시퀀스에서 라벨 둘이 겹쳤다(라이트 테마에서 뚜렷) | 버전 확인 라벨을 왼쪽으로 옮겼다  |
| `.note`(함정 블록)만 `max-width: 74ch` 라 좁았다      | 지웠다 — 표·도해와 같은 폭이 된다 |

세 번째는 내용이 아니라 **레이아웃**이라 텍스트 대조로는 영영 안 나온다.

### 링크로 열리게 했다 — GitHub Pages

README 의 해부도 링크는 눌러도 소스만 보였다. 저장소 화면은 HTML 을 렌더링하지 않는다.
`.github/workflows/pages.yml` 이 `docs/` 를 그대로 Pages 에 올린다.

<https://chane81.github.io/mfa-nextjs/anatomy.html>

- **파일 하나가 아니라 `docs/` 통째**로 올린다. 해부도가 각 절에서 `./02-architecture/*.md`
  를 상대 경로로 가리키기 때문이다.
- Pages 는 그 마크다운을 **렌더링하지 않는다.** 그래서 해부도 안에 스크립트를 하나 더
  뒀다 — 호스트가 `github.io` 일 때만 `.md` 상대 링크를 GitHub blob 주소로 바꾼다.
  로컬(`file://`)로 열었을 때는 옆 파일을 그대로 가리켜야 하므로 건드리지 않는다.
- `docs/index.html` 은 해부도로 보내는 리다이렉트다. Pages 루트가 404 가 되지 않게.

`enablement: true` 로 워크플로가 Pages 를 켜는 길은 안 썼다 — 그 입력은 `GITHUB_TOKEN`
으로는 안 되고 별도 PAT 을 요구한다(공식 `action.yml` 확인). 저장소 설정에서 한 번 켜는
편이 토큰 하나를 더 만드는 것보다 싸다.
