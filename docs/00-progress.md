...
브랜치를 파고 `partialPrefetching: true` 로 직접 쟀다. 전문: `docs/04-experiments/04-partial-prefetching.md`

### 잰 것

전송량은 일관되게 줄었다 — `/` −26%, `/lab` −30%, `/products/kb-001` −25%(enc 기준).
목적지별로는 `/lab/isr` 이 −56% 로 가장 컸다. 깨진 건 없었다(ADR-014 유지, 클릭
내비게이션 정상, 프리페치 응답에 세션 장바구니 미포함, 콘솔 에러 0).

**라우트 표는 완전히 동일하다.** 프리페칭은 빌드 산출물이 아니라 런타임 동작이라,
빌드 표만 봐서는 이 플래그가 켜졌는지조차 알 수 없다.

### 진짜 발견 — 요청 건수가 하나도 안 줄었다

부분 프리페칭의 간판 이득은 "같은 라우트로 가는 링크 N 개 → App Shell 1개 공유"인데,
**이 저장소에는 그 패턴이 하나도 없다.** `<Link>` 다섯 개가 전부 서로 다른 라우트다.

N:1 이 나올 유일한 자리인 상품 목록 → `/products/[id]` 는 `<Link>` 가 아니라
`<button onClick={onSelect}>` + host 의 `router.push` 다. remote 가 host 의 라우터를
몰라야 하기 때문이고(ADR-013), 그 대가로 **그 자리는 프리페치가 아예 안 일어난다** —
실측에서 `/` 의 프리페치 목적지에 `/products/*` 가 한 건도 없었다.

MFA 의 일반적 트레이드오프다. 라우팅을 host 가 소유하면 remote 가 그리는 목록은 `<a>` 가
아니게 되고, 프리페치·새 탭·링크 복사를 같이 잃는다. host 가 `onSelect` 대신 **링크를
렌더하는 함수**를 내려보내면 풀리지만(계약이 하나 는다) 지금 할 값은 아니라 기록만 했다.

### 켜지 않는 이유

얻는 게 페이지당 2~4KB 고 잃는 건 없지만, 이 플래그가 바꾸는 건 이 저장소가 검증하려는
주장(런타임 MF + remote SSR)과 **무관하다.** 설정이 하나 늘면 나중에 MF 증상을 볼 때
배제할 변수가 하나 는다. 켤 조건도 같이 적어뒀다 — 상품 카드가 진짜 링크가 되는 순간.

### 방법론 함정 — `curl` 로 프리페치를 흉내내면 안 된다

`RSC: 1` + `Next-Router-Prefetch: 1` 만 보내면 `◐` 라우트가 **404** 를 준다.
`next-router-state-tree` 가 빠져서고, 버그가 아니라 합성 요청이 불완전한 것이다.
그걸 모르고 봤으면 없는 장애를 쫓았다. 응답의 `vary` 가 실제 축을 알려준다 —
`rsc, next-router-state-tree, next-router-prefetch, next-router-segment-prefetch`.
브라우저가 실제로 보낸 것을 Resource Timing 으로 재는 편이 언제나 맞다.

검증 판별자도 한 번 틀렸다. `/` 에서 "B 세션 응답에 A 의 값(189,000원)" 이 참으로 나왔는데
오염이 아니라 `kb-001` 의 상품 가격이 189,000원이라 홈 그리드에 늘 찍히는 것이었다.
판별자는 그 라우트에서 **장바구니에만** 나타나는 값으로 골라야 한다.
