...
## 2026-08-20 (13차) — 하이드레이션 구간을 전환 애니메이션으로 바꾼다

새로고침하면 헤더 배지와 장바구니 패널이 한 번 깜빡였다. 원인을 끝까지 파 보니
저장소가 느린 게 아니라 **React 가 하이드레이션 렌더에서 서버 스냅샷을 쓰는 한 프레임**이었다.
없앨 수 없는 구간이라, 감추는 대신 **전환 구간으로 쓰기로** 했다.

### 한 일

- [x] `useHydrated()` 신설(`packages/store/src/hooks/use-hydrated.ts`).
      `useSyncExternalStore` 로 하이드레이션 커밋 경계만 노출한다 —
      서버·첫 렌더 `false`, 커밋 후 `true`.
      **도메인 폴더가 아니라 `hooks/` 에 둔다** — 장바구니도 zustand 도 참조하지 않고,
      브라우저에만 있는 값을 쓰는 자리는 전부 같은 경계를 갖는다
- [x] 전환은 **전부 Tailwind 유틸리티**로 적었다. 크기는
      `grid-rows-[0fr]` → `data-[open=true]:grid-rows-[1fr]`(가로는 `grid-cols-*`),
      흐림은 `blur-xs`, 자식은 `*:overflow-hidden`, 움직임 줄이기는 `motion-reduce:`
- [x] CSS 에 남긴 건 **유틸리티로 못 적는 키프레임 둘**뿐. `@theme` 에
      `--animate-cart-line` · `--animate-cart-pop` 으로 등록해 `animate-*` 유틸리티가 생긴다
- [x] 곡선은 공유 토큰 `--ease-reveal` (`theme.css`). 배지·목록·흐림이 같은 곡선을 탄다
- [x] 반복되는 유틸리티 조합은 컴포넌트로 묶었다 —
      `apps/remote-cart/src/components/Reveal.tsx` (`Reveal` · `settlingClass`)
- [x] `CartBadge` · `CartPanel` · `CheckoutFlow` 를 그 위에 다시 배선.
      **하이드레이션 전에는 접어 두고**, 커밋에서 실제 내용으로 한 번 펼친다
- [x] 접히지 않는 자리(합계 줄)는 상자 단위로 처리. 확정 전에는 패널 전체가
      `opacity-50 blur-xs`(4px) 상태고, 값이 확정되는 순간 또렷해진다 —
      `0개 · 합계 0원` 이 읽히지 않는다
- [x] `prefers-reduced-motion: reduce` 면 전환 없이 최종 상태만
- [x] ADR-014, known-issues E-1 · E-2(+ 증상 색인 2줄)

### 실측 (CDP 로 rAF 마다 표본)

| 자리           | 고치기 전                | 고친 뒤                    |
| -------------- | ------------------------ | -------------------------- |
| 헤더 배지 폭   | 97.8 → 187.6px 한 프레임 | 같은 구간을 300ms 곡선으로 |
| 패널 본문 높이 | 0 → 206.5px 한 프레임    | 같은 구간을 300ms 곡선으로 |

커밋 시점은 로드 후 35~60ms. 그 한 프레임에 두 자리가 동시에 튀는 게 깜빡임의 정체였다.

### 판단

- **왜 숨기지 않았나.** 스켈레톤도, 색 없는 자리표시자도 먼저 해 봤고 둘 다 더 나빴다.
  한 프레임짜리에 로딩 UI 를 붙이면 번쩍임이 되고, **줄 수를 서버도 첫 렌더도 모르니**
  자리 크기를 실제와 맞추는 건 원리상 불가능하다 — 층 이동이 그대로 남는다
- **왜 쿠키로 안 옮겼나.** 서버가 장바구니를 알게 되면 구간 자체가 사라지지만,
  `cookies()` 를 읽는 라우트는 프리렌더를 잃고 "브라우저 안에서만 사는 상태"라는
  이 실험의 전제가 바뀐다. 한 프레임 대가로는 과하다
- **`height: auto` 는 전환이 안 걸린다.** `grid-template-rows: 0fr → 1fr` 이
  내용 높이를 미리 알 필요가 없는 유일한 방법이다. 배지처럼 폭이 튀는 자리는
  `grid-template-columns` 로 같은 장치를 쓴다
- **빈 문구를 먼저 띄우지 않는다.** 장바구니가 비어 있지 않은 사람에게 사실이 아닌
  문장을 보여 줬다 접으면 층 이동이 양방향 두 번이 된다. 접힌 채로 시작하면 한 번이다
- **CSS 파일에는 유틸리티로 못 적는 것만 둔다.** 전환 규칙을 CSS 로 내려보내면
  마크업에서 안 보이게 된다. 못 적는 건 키프레임뿐이었고, 그건 `@theme` 등록으로 해결된다.
  긴 조합의 중복은 CSS 클래스가 아니라 컴포넌트로 막는다 — 규칙은 `.claude/rules/styling.md`
- **접히지 않는 자리는 상자로 묶는다.** 합계 줄은 목록과 달리 접히지 않아 `0원` 이 남는다.
  그 자리마다 장치를 하나씩 더 붙이는 대신 상자 전체를 흐리게 뒀다. 값을 감추는 게 아니라
  "아직 확정 전"을 상태로 보여주는 것이라, 초점이 맞는 동작이 의미와도 맞는다

### 다음

- catalog remote 도 같은 구간을 갖는지 확인(지금은 서버 데이터라 해당 없음)
