...
| 검사가 대조한다         | `docker-context.test.ts`                     | 예             |
...
- ~~⭕ 배포 전에 이미지 빌드를 본다. 예전에는 배포가 최초 검증이었다.~~
  **39차에 뒤집혔다(ADR-023).** CI 의 docker job 은 게이트가 아니어서 아무것도 막지
  못했고, 같은 이미지를 Dokploy 가 어차피 다시 빌드했다. 목록 드리프트는
  `docker-context.test.ts` 가 오프라인으로 계속 잡는다.
...
- ~~❌ CI 가 길어진다. docker job 셋이 늘었다 — remote 이미지 전체 + host 의 `deps`.~~
  **이 대가도 같이 사라졌다(ADR-023).** 덧붙여 이 걱정 자체가 과했다 — GHA 캐시가 먹어서
  실측은 총 1분 남짓이었다. 뺀 이유는 비용이 아니라 **막는 게 없어서**다.
...
## ADR-023 — CI 에서 배포 이미지를 빌드하지 않는다 (ADR-021 의 한 갈래를 뒤집는다)

**날짜** 2026-09-06 (39차) · **상태** 채택 · **대체** ADR-021 의 "배포 전에 이미지 빌드를 본다"

### 맥락

38차에 CI 에 docker job 셋을 만들었다(`docker-targets` · `docker-remotes` · `docker-host`).
근거는 I-10 이었다 — `deps` 목록 드리프트를 아무도 못 보고 **배포가 최초 검증**이었다.

39차에 "이 job 들이 실제로 무엇을 막나" 를 물었더니 답이 **아무것도** 였다.

| 사실                                                                            | 확인 방법                                        |
| ------------------------------------------------------------------------------- | ------------------------------------------------ |
| `deploy` 에 `needs: ci` 도 `workflow_run` 도 없다                               | `.github/workflows/deploy.yml` 의 `on:`          |
| `main` 에 브랜치 보호가 없다                                                    | `gh api …/branches/main/protection` → 404        |
| 그래서 두 워크플로가 같은 push 에 **나란히** 뜬다                               | 위 둘의 귀결                                     |
| `push: false` 라 만든 이미지를 버린다. Dokploy 가 자기 호스트에서 다시 빌드한다 | job 정의 · `dokploy-deploy` 가 API 만 부른다     |
| 이미지가 깨지면 배포가 실제로 죽는다                                            | `dokploy-deploy` 가 `deployment.all` status 폴링 |

즉 **CI 가 빨개도 배포는 그대로 나갔다.** 같은 빌드를 두 번 하고, 그중 한 벌은 버렸다.

### 결정

**뺀다.** 이미지 검증은 `deploy` 가 한다.

세 갈래 중 이 갈래만 뒤집는다. ADR-021 의 원칙("파생할 수 있으면 파생하고, 못 하면
어긋남을 죽는 검사로 바꾼다")은 그대로다 — I-10 이 겨눈 목록 드리프트는
`scripts/docker-context.test.ts` 가 `pnpm test` 에서 **오프라인으로** 잡고, 그쪽이
어느 파일에 무슨 줄을 넣어야 하는지까지 말한다. 없어진 건 "실제 이미지를 만들어 본다"
한 겹뿐이고, 그 겹은 배포가 이미 지고 있었다.

### 기각한 대안 — 게이트로 만든다

`main` 에 브랜치 보호 + required status checks 를 걸어 실제 게이트로 만드는 안을 봤다.
비용도 쌌다 — GHA 캐시가 먹어서 실측 총 1분 남짓이다(38차 주석의 "길다" 는 틀렸다).

기각한 이유는 **이 저장소의 작업 방식**이다. main 에 들어온 머지 10건 중 PR 을 거친 건
하나뿐이고 나머지는 로컬 `Merge branch` 다. 그 경로에서는 required check 가 아무것도 안
막으면서 push 만 거부한다. 실험 저장소의 진행 속도를 그 대가로 내줄 이유가 없다.

PR 흐름으로 굳어지면 다시 연다. 그때는 job 을 되살리는 게 아니라 **브랜치 보호부터** 건다 —
게이트 없는 검사 job 이 이번에 만든 상태다.

### 결과

- ⭕ 같은 이미지를 두 번 빌드하지 않는다.
- ⭕ CI 의 job 이 셋으로 돌아가 "무엇이 깨졌나" 가 체크 이름에서 다시 바로 읽힌다.
- ❌ **PR 에서는 이미지 상태를 못 본다.** 이미지가 깨진 브랜치가 main 에 들어갈 수 있고,
  그때 `deploy` 의 remote job 이 죽는다. 프로덕션은 옛 컨테이너 그대로라 안전하지만
...
`SHARED_DEPLOY_PATHS` 에 `packages/` 가 통짜로 들어 있다(ADR-021). 그래서 **테스트 헬퍼
한 줄을 고쳐도 remote 둘과 host 가 전부 재배포된다.**

이 대가를 그대로 진다. 구멍을 파는 안(`DEPLOY_IGNORED_PATHS` — "이미지 안에서 실행되지
않는 경로는 배포를 안 부른다")을 만들어 봤다가 되돌렸다.

기각한 이유는 **틀리는 방향**이다. 두 선택의 사고가 대칭이 아니다.

| 선택        | 틀렸을 때 무슨 일이 나나            | 언제 알게 되나            |
| ----------- | ----------------------------------- | ------------------------- |
| 통짜로 둔다 | 안 바뀐 이미지를 다시 만들어 올린다 | 즉시, 그리고 무해하다     |
| 구멍을 판다 | **바뀐 코드가 배포에 안 실린다**    | "고쳤는데 반영이 안 된다" |

그리고 이 패키지 이름은 `@mfa/utils` 다. **범용 이름은 범용으로 쓰이게 된다** — 프로덕션
유틸이 하나 들어오는 순간 구멍은 조용한 사고가 된다. 그때 구멍을 메우는 걸 기억해야
성립하는 안전은 안전이 아니다.

재배포 비용은 이 저장소에서 시간뿐이다(Dokploy 가 자기 호스트에서 빌드하고, 세 앱은
독립이라 병렬로 나간다). 그쪽이 훨씬 싸다.

`scripts/deploy-targets.test.ts` 가 `packages/utils/` 변경도 전체 배포를 부르는지 본다 —
...
- ⭕ 배포 판별에 예외가 없다. `packages/` 아래는 전부 배포를 부른다 — 규칙이 하나다.
- ❌ 세 Dockerfile 의 `COPY … package.json` 목록이 한 줄씩 늘었다
  (`docker-context.test.ts` 가 강제하므로 잊을 수는 없다).
- ❌ **테스트 헬퍼를 고치면 세 앱이 재배포된다.** 산출물은 안 바뀌는데 파이프라인은 돈다.
  의도한 대가다(위 표) — 아프면 그때 줄일 자리는 예외 목록이 아니라 이미지 캐시다.
- ❌ 이름이 `@mfa/utils` 라 **테스트 전용이라는 게 이름에 없다.** 지금은 그게 오히려
  안전한 쪽으로 작동한다 — 프로덕션 유틸이 들어와도 배포 판별이 이미 맞다.

## ADR-026 — `@mfa/utils` 는 범용 패키지다. 테스트 헬퍼는 그 안의 한 갈래일 뿐이다

**날짜** 2026-09-18 (42차) · **상태** 채택 · **범위 확정** ADR-025

### 맥락

ADR-025 가 `@mfa/utils` 를 만들 때 들어간 것이 테스트 헬퍼 넷뿐이라, 그 기록이 패키지를
**테스트 헬퍼 패키지**로 기술한다. 그래서 42차에 `hueVar` 를 둘 자리를 고를 때
"여기는 테스트 전용이니 안 된다" 는 판단이 먼저 나왔다 — **기록이 범위를 좁게 말한 탓이다.**

ADR-025 자신은 그렇게 안 봤다. 마지막 줄이 이렇다.

> 이름이 `@mfa/utils` 라 테스트 전용이라는 게 이름에 없다. 지금은 그게 오히려 안전한
> 쪽으로 작동한다 — **프로덕션 유틸이 들어와도 배포 판별이 이미 맞다.**

배포 구멍(`DEPLOY_IGNORED_PATHS`)을 기각해 뒀기 때문에, 이 패키지에 런타임 코드가
들어오는 것은 **이미 대비된 경우**다. `packages/` 는 통짜로 "바뀌면 전부 배포" 다.

### 결정

`@mfa/utils` 는 **소유자가 없는 범용 유틸**의 자리다. 어느 앱·패키지의 도메인에도
속하지 않고 두 곳 이상이 쓰는 것이 여기 온다. `src/test/` 는 그중 한 갈래일 뿐이다.

- **배럴은 여전히 안 둔다**(ADR-025). 모듈마다 필요한 환경이 다르다 —
  `style/hue` 는 React 타입, `test/cookies` 는 DOM, `test/signing` 은 node.
  서브패스가 그 비대칭을 구조적으로 막는다.
- **빌드도 여전히 안 둔다.** `exports` 가 소스 `.ts` 를 직접 가리키고 세 앱의 번들러가
  그대로 삼킨다(`@mfa/remote-config` 와 같은 성질). 실측: `style/hue` 를 host(Turbopack)와
  cart(Rsbuild) 양쪽 번들에 태우고 `pnpm build` 통과 — `transpilePackages` 도 안 건드렸다.
- 런타임에 쓰는 소비처는 `dependencies` 에 적는다. 테스트에서만 쓰면 `devDependencies` 다.
  그 구분이 이 패키지에서 유일하게 남는 "테스트인가 아닌가" 다.

### 첫 사례 — `hueVar`

`@mfa/ui` 안의 비공개 함수였다. `--hue` 라는 CSS 변수 이름과 `as CSSProperties` 캐스팅을
감추는데, `Panel`·`Badge` 로 감쌀 수 없는 자리 셋이 그걸 리터럴로 베껴 갔다.

`@mfa/ui` 로 export 하는 안을 먼저 잡았다가 옮겼다. **그 패키지는 컴포넌트를 내보내는
자리**고, 소비처 셋 중 둘은 프리미티브를 안 쓰면서 이 값만 필요하다 — 거기 두면
"컴포넌트를 안 쓰는데 컴포넌트 패키지를 의존한다" 가 된다.

### 기각한 대안 — `@mfa/tailwind-config` 에 둔다

변수 **이름의 원본**이 그 패키지의 `theme.css` 라 결이 맞았다. 기각한 이유는 그 패키지가
**CSS 전용**이라는 것이다 — `exports` 가 `./theme.css` 와 `./postcss` 둘뿐이고 TS 소스도
빌드도 없다. 함수 하나 때문에 JS 진입점이 생기면, 지금 CSS 로만 쓰는 세 앱에 새 축이 붙는다.
얻는 건 "이름이 정의 옆에 있다" 하나뿐이라 대가가 더 크다.

### 결과

- ⭕ 둘 이상이 쓰는 유틸에 **갈 곳이 생겼다.** 전에는 도메인 패키지에 억지로 넣거나
  복제하는 둘뿐이었고, 실제로 복제 쪽이 세 번 일어났다.
- ⭕ ADR-025 의 배포 판정이 그대로 맞다 — 고칠 것이 없다.
- ❌ 이름이 범용이라 **아무거나 들어올 수 있다.** 도메인이 있는 것(장바구니·remote 배치)은
  자기 패키지로 간다는 선을 여기 적어 둔다. 이 패키지는 "소유자가 없다" 가 조건이다.
