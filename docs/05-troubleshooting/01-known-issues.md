...
늦는 쪽은 React 다. `useStore` 는 내부적으로 `useSyncExternalStore` 를 쓰고,
하이드레이션 렌더에서는 **서버 스냅샷**(`getInitialState()` = `lines: []`)을 쓴다.
서버 HTML 과 첫 클라이언트 렌더가 달라지면 안 되기 때문이다. 즉 **이미 아는 값을 일부러
한 프레임 안 쓴다.** ADR-012 가 "SSR 은 그대로 안전하다"고 적은 그 성질의 뒷면이다.

CDP 로 rAF 마다 표본을 뜨면 이렇게 보인다(항목 3개짜리 장바구니, 프로덕션 빌드).

```
  t(ms)  badgeW  panelBodyH
    9.2    97.8         0     ← 서버 스냅샷. 빈 장바구니
   34.6    97.8         0     ← 하이드레이션 커밋 직전
   51.2   119.1      49.1     ← 값이 들어옴
```

고치기 전에는 이 자리가 한 프레임 만에 `97.8 → 187.6px`, `0 → 206.5px` 로 튀었다.
**깜빡임의 정체는 색이 아니라 층 이동이다.**

진단할 때 헷갈리지 않는 방법: `localStorage` 를 의심하지 말고
`useCart.getState().lines` 를 첫 스크립트에서 찍어 본다. 이미 들어 있다.
...
1. **스켈레톤** — 회색 상자가 "로딩 중"이 아니라 **번쩍임**으로 읽혔다. 한 프레임 남짓
   보이는 것에 로딩 UI 를 붙이면 없는 것보다 시끄럽다. 게다가 스켈레톤 1줄 대 실제 2줄이라
   층 이동이 오히려 하나 늘었다.
2. **색 없는 자리표시자** — 소리는 줄었는데 크기가 여전히 안 맞았다. 당연하다.
   **줄 수는 서버도, 첫 클라이언트 렌더도 모른다.** 자리 크기를 실제와 맞추는 건 원리상 불가능하다.

결론은 방향을 뒤집는 것이었다 — 가리지 말고 **그 구간을 전환 구간으로 쓴다**(ADR-014).
`height: auto` 는 transition 이 안 걸리므로 `grid-rows-[0fr]` → `data-[open=true]:grid-rows-[1fr]`,
배지처럼 폭이 튀는 자리는 `grid-cols-*` 로 같은 장치를 쓴다.

접히지 않는 자리는 따로 다룬다. 목록은 접혀도 **합계 줄은 남아** `0개 · 합계 0원` 이
한 프레임 노출된다. 그 자리만 또 가리면 장치가 계속 늘어나므로, 상자(패널) 전체에
`opacity-50 blur-xs`(4px)를 걸고 값이 확정되는 순간 푼다.

⚠️ 이때 **하이드레이션 전에 "담긴 상품이 없습니다"를 띄우면 안 된다.** 장바구니가 비어
있지 않은 사람에게 사실이 아닌 문장을 한 프레임 보여 줬다 접는 꼴이고, 접혔다 펴지는
층 이동이 양방향으로 두 번 일어난다. 접힌 상태에서 시작하면 한 방향으로 한 번만 움직인다.
