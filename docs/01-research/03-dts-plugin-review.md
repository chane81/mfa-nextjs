...
## 8. 후속 — 켜 봤고, 결론이 뒤집혔다 (27차, `feat/mf-dts`)

이 문서는 **검토만** 했고 "도입 보류" 로 끝냈다. 27차에 실제로 배선해보니
**보류 판정의 근거 하나가 틀렸다.** 전문: [진행 상황 27차](../00-progress.md).

| 이 문서의 예측                           | 실측                                                           |
| ---------------------------------------- | -------------------------------------------------------------- |
| 콘솔 에러는 `dev` 소관이다               | ⭕ 맞다                                                        |
| 플러그인 없는 host 도 타입을 소비한다    | ⭕ 맞다. dev · 빌드 산출물 양쪽에서 돈다                       |
| typecheck 가 remote 기동을 요구하게 된다 | ❌ **틀렸다.** `@mf-types` 를 커밋하면 네트워크 0회를 유지한다 |
| 정보가 중복이라 DTS 가 알려줄 게 없다    | ⚠️ **배치를 바꾸면 아니다.** 아래                              |

### 4절의 "정보 중복" 은 배치의 결과였지 DTS 의 한계가 아니었다
...
이 문서는 이걸 "DTS 가 알려주는 정보의 대부분을 host 는 이미 알고 있다" 로 읽었다.
맞는 관찰이지만 **원인 진단이 반대였다.** host 가 이미 알고 있는 이유는 정보가 원래
중복이어서가 아니라, **props 선언을 계약 패키지에 뒀기 때문**이다.

그래서 27차에는 소유권을 옮겼다. props 는 remote 의 expose 파일 옆으로 가고,
`RemoteModuleMap` 은 host 가 DTS 로 받은 타입으로 조립한다. 그러자 remote 가 필수 prop 을
늘렸을 때 **host 의 실제 호출부가 컴파일 에러로 죽었다** — 이 문서가 얻을 수 없다고 본
바로 그 검증이다.

### 5절의 "CI 순서 의존" 도 피할 수 있었다

`consumeTypes` 가 HTTP 를 쓰는 건 맞지만, **받은 결과를 커밋하면** 그 의존이 검증
단계에서 빠진다. `pnpm typecheck` 는 커밋된 `@mf-types` 를 읽으므로 네트워크 0회다.

낡을 위험은 남는데, 그건 CI 가 `pnpm build` 뒤에 `pnpm mf:types` 를 돌리고
`git diff --exit-code` 로 본다. 순서 의존이 **모든 검사**에서 **CI 한 단계**로 줄었다.

### 그대로인 것

- **SSR 경로는 DTS 밖이다**(4절). 다만 호출부가 `loadRemoteModule` 하나로 통일돼 있고
  그 반환 타입이 `RemoteModuleMap` 이라, 그 맵의 내용이 DTS 에서 오는 지금은
  서버 경로의 타입도 remote 에서 온다.
- **`extractThirdParty` 는 이 저장소에서 안 된다**(known-issues I-3).
  `third-party-dts-extractor` 가 `require.resolve` 를 쓰는데 `@mfa/contracts` 는
  `require` 조건이 없는 ESM 전용 패키지다. remote 가 다른 저장소로 나가는 날
  계약 패키지에 CJS 진입점을 붙이는 것이 첫 작업이다.

### 판정 정정

**도입.** 7절의 재검토 조건("다른 저장소로 나갈 때")을 기다릴 필요가 없었다 —
얻는 것은 조건이 아니라 **배치**에 달려 있었다.
